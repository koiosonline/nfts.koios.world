...
const BadgeClaimModal = (items: IERC721MetadataModel[]) => {
  ...
  const { data: userBadges, isLoading: isUserLayerLoading } =
    useUserBadges(user);
  ...
  useEffect(() => {
    if (data && userBadges) {
      ...
      for (let item of dataArray) {
        ...
        if (foundMetadata >= 0 && !userBadges.includes(item.tokenId)) {
        ...
        <div className="flex h-[85%] w-full justify-between rounded bg-zinc-900 p-2 pt-5 md:p-10">
          <div className="flex h-full w-full flex-col overflow-y-auto">
            {fil.map((item: IERC721MetadataModel, index: number) => (
              ...
              >
                <h1 className="font-heading text-xl text-white">{item.name}</h1>
                <ClaimBadgeButton tokenId={item.tokenId} />
              </div>
            ))}
          </div>
        </div>
